...
const CACHE = 'mri-sim-v1';
const CORE = [
  './',
  './index.html',
  './manifest.json',
  './mri_calibration_export-v2.json',
  ...
  e.waitUntil(
    caches.open(CACHE).then(cache =>
      cache.addAll(['./index.html', './manifest.json'])
        .then(() => cache.add('./mri_calibration_export-v2.json').catch(() => {}))
        .then(() => cache.add('./mri_planning_2026-03-01.json').catch(() => {}))
