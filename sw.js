...
// SW VERSION: v33 — must match app version; bump CACHE name on every index.html change
...
// Always fetch from network (bypassing SW cache) — update cache on success,
// fall back to cache only when truly offline. Used for data/manifest.json.
async function networkFirstNoCache(request) {
  try {
    const res = await fetch(request, { cache: 'no-store' });
    if (res.ok) {
      // Clone and store in SW cache for offline fallback
      const clone = res.clone();
